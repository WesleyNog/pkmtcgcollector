...
class TriumphantMetrics extends StatefulWidget {
  TriumphantMetrics({super.key});

  @override
  State<TriumphantMetrics> createState() => _TriumphantMetricsState();
}

class _TriumphantMetricsState extends State<TriumphantMetrics> {
  int touchedIndex = -1;
  ...
  int obtido(String pack, {String tipo = "Normal"}) {
    if (pack == "Total") {
      return pokemonList
          .where((pokemon) =>
              pokemon["obtido"] == true && pokemon["seasson"] == "Triumphant")
          .length;
    }
  ...
  int totalPokemon(String pack, {String tipo = "Normal"}) {
    if (pack == "Total") {
      return pokemonList
          .where((seasson) => seasson["seasson"] == "Triumphant")
          .length;
    }
      ...
      child: Column(
        children: [
          ...
          createDataTable(labels: [
            ...
            [
              "Arceus",
              "${obtido("Arceus")}/${totalPokemon("Arceus")}",
              "${percentBuster("Arceus")}%"
            ],
            [
              "Total",
